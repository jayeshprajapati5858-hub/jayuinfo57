...
import { Plus, Package, Check, X, ArrowLeft, Printer, Trash2, Tag, Upload, Image as ImageIcon, Server, HardDrive, Loader2 } from 'lucide-react';
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      try {
        ...
        let errorMsg = "Photo upload fail thayo.";
        if (error.code === 'storage/unauthorized') {
          errorMsg = "Permission nathi! (Storage Rules check karo)";
        } else if (error.code === 'storage/retry-limit-exceeded') {
          errorMsg = "Network problem che, fari try karo.";
        } else if (error.code === 'storage/canceled') {
          errorMsg = "Upload cancel thayu.";
        }

        // Fallback strategy: Use random image if storage fails
        imageUrl = `https://picsum.photos/400/400?random=${Date.now()}`;
        alert(`${errorMsg}\n\nTamare Firebase Console ma 'Storage' -> 'Rules' ma javu padshe ane 'allow read, write: if true;' karvu padshe.\n\n(Haal random photo mukayo che)`);
      }
    } else if (!imageUrl) {
    ...
    const product: Product = {
      id: '', // ID will be generated by Firestore in App.tsx
    ...
    // Reset Form
    setNewProduct({ name: '', price: '', category: Category.COVER, description: '', image: '', stock: 50 });
    setImageFile(null);
      ...
      <div className="bg-gray-900 text-white p-4 sticky top-0 z-10 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto justify-between md:justify-end">
             <div className="px-3 py-1 rounded bg-blue-800 text-blue-200 text-xs font-mono">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
              ...
              <form onSubmit={handleProductSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex gap-4 items-start">
                    ...
                    <div className="flex-1 space-y-3">
                        ...
                        <input 
                            type="text"
                            value={imageFile ? '' : newProduct.image}
                            disabled={!!imageFile}
                            onChange={e => setNewProduct({...newProduct, image: e.target.value})}
                            placeholder="https://example.com/image.jpg"
                            className="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none text-sm text-gray-600 disabled:opacity-50"
                        />
                    </div>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-gray-900 text-white py-3 rounded-xl font-bold hover:bg-primary transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  {isUploading ? <Loader2 className="animate-spin" /> : <Plus size={20} />} 
                  {isUploading ? 'Uploading & Adding...' : 'Add Product'}
                </button>
              </form>
            </div>
